...
function calculate() {
  ...
  if (!metric) {
    if (calculatedBMI > 18.5 && calculatedBMI < 25) {
      message =
        idealWeight +
        `Your ideal weight is between ${idealWeightMinInLbs.toFixed(
          2
        )} - ${idealWeightMaxInLbs.toFixed(2)} lbs.`;
    ...
    if (calculatedBMI > 18.5 && calculatedBMI < 25) {
      message =
        idealWeight +
        `Your ideal weight is between ${idealWeightMinInKgs.toFixed(
          2
        )} - ${idealWeightMaxInKgs.toFixed(2)} kg.`;
    } else if (calculatedBMI < 18.5) {
      message =
        underweight +
        `Your ideal weight is between ${idealWeightMinInKgs.toFixed(
